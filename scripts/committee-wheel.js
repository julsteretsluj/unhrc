 ...
 * On Enter for current committee (UNHRC), hides wheel and shows payment gate.
 */
(function () {
  ...
  function render() {
    ...
    var logoSrc = c.logo ? "assets/committee-logos/" + c.logo + "?v=20260803c" : "";
    ...
    root.innerHTML =
      ...
      (isCurrent ? "Continue into UNHRC" : "Continue to selected committee") +
      '" title="' +
      ...
      "</button>" +
      (isCurrent
        ? '<p class="wheel-hint">UNHRC committee portal — topics, allocations, resources</p>'
        : "") +
      "</div>" +
  ...
  function enter() {
    ...
    if (c.isCurrent) {
      ...
      if (paymentGate) {
        ...
        window.dispatchEvent(new CustomEvent("unhrc:show-payment"));
      } else if (siteApp) {
  ...
  function bind() {
    ...
    wheelEl.addEventListener("pointermove", function (e) {
      ...
      if (isDragging && rotator) {
        ...
        if (img && dc.logo) img.src = "assets/committee-logos/" + dc.logo + "?v=20260803c";
